...
/* Device registers */
#define BG_RED(x) ((x))
#define BG_GREEN(x) ((x)+3)
#define SQUARE_X(x) ((x)+4)
#define SQUARE_Y(x) ((x)+6)
...
static void write_background(vga_ball_color_t *background) {
  iowrite16((background->green << 8) | background->red, BG_RED(dev.virtbase));
}

static void move(int* xpos, int* ypos, int* xdelt, int* ydelt)
{

	iowrite16(*xpos, SQUARE_X(dev.virtbase));
	iowrite16(*ypos, SQUARE_Y(dev.virtbase));
...
static long vga_ball_ioctl(struct file *f, unsigned int cmd,
  ...
  case VGA_BALL_READ_BACKGROUND:
	...
	case VGA_BALL_MOVE_SQUARE:
		if (copy_from_user(&vla, (vga_ball_arg_t *) arg,
				   sizeof(vga_ball_arg_t)))
			return -EACCES;
		move(&vla.xpos, &vla.ypos, &vla.xdelt, &vla.ydelt);
		break;

  case VGA_BALL_READ_BLANK:
